"""Report writers for consolidate execution."""
...
from datamapx.consolidate.config import ConsolidateConfig
from datamapx.consolidate.errors import ConsolidateErrorRow, ConsolidateSkippedRow
from datamapx.consolidate.runner import ConsolidateResult
...
def resolve_consolidate_report_paths(
    ...
    *,
    ...
    """Resolve consolidate report output paths."""
...
def write_consolidate_reports(
    ...
    *,
    ...
    """Write consolidate error/skipped/summary reports."""

    report_paths = resolve_consolidate_report_paths(
        config,
        config_path,
        reports_dir,
...
def _write_errors_csv(path: Path, error_rows: list[ConsolidateErrorRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
...
def _build_summary_payload(
    ...
    return {
        ...
        "outputs": [
            {
                ...
                "columns": output.df.columns.tolist(),
            }
        ...
        "notes": {
            ...
            "group_by": config.consolidate.group_by,
        },
    }
...
def _error_row_payload(row: ConsolidateErrorRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "input_name": row.input_name,
        "row_number": row.row_number,
...
def _skipped_row_payload(row: ConsolidateSkippedRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "row_number": row.row_number,
        "reason": row.reason,
