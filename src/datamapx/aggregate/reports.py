"""Report writers for aggregate execution."""
...
from datamapx.aggregate.config import AggregateConfig
from datamapx.aggregate.errors import AggregateErrorRow, AggregateSkippedRow
from datamapx.aggregate.runner import AggregateResult
...
def resolve_aggregate_report_paths(
    ...
    *,
    ...
    """Resolve aggregate report output paths."""
...
def write_aggregate_reports(
    ...
    *,
    ...
    """Write aggregate error/skipped/summary reports."""

    report_paths = resolve_aggregate_report_paths(
        config,
        config_path,
        reports_dir,
...
def _write_errors_csv(path: Path, error_rows: list[AggregateErrorRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
...
def _write_skipped_csv(path: Path, skipped_rows: list[AggregateSkippedRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
            "row_number": row.row_number,
...
def _build_summary_payload(
    ...
    return {
        ...
        "notes": {
            ...
            "group_by": config.aggregate.group_by,
...
def _error_row_payload(row: AggregateErrorRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "input_name": row.input_name,
        "row_number": row.row_number,
...
def _skipped_row_payload(row: AggregateSkippedRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "row_number": row.row_number,
        "reason": row.reason,
