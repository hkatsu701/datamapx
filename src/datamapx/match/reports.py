"""Report writers for match execution."""
...
from datamapx.match.runner import MatchResult
...
def resolve_match_report_paths(
    config: MatchConfig,
    config_path: Path,
    ...
    *,
    ...
    """Resolve match report output paths."""
...
def write_match_reports(
    ...
    *,
    ...
    """Write match error/skipped/summary reports."""

    report_paths = resolve_match_report_paths(
        config,
        config_path,
        reports_dir,
...
def _write_errors_csv(path: Path, error_rows: list[MatchErrorRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
...
def _write_skipped_csv(path: Path, skipped_rows: list[MatchSkippedRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
            "row_number": row.row_number,
...
def _build_summary_payload(
    ...
    return {
        ...
        "notes": {
            ...
            "match_keys": config.match.keys,
            "match_output_column": config.match.output_column,
        },
    }
...
def _error_row_payload(row: MatchErrorRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "input_name": row.input_name,
        "row_number": row.row_number,
...
def _skipped_row_payload(row: MatchSkippedRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "row_number": row.row_number,
        "reason": row.reason,
