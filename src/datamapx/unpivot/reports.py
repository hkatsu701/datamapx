"""Report writers for unpivot execution."""
...
from datamapx.unpivot.runner import UnpivotResult


def resolve_unpivot_report_paths(
    ...
    *,
    ...
    """Resolve unpivot report output paths."""
...
def write_unpivot_reports(
    ...
    *,
    ...
    """Write unpivot error/skipped/summary reports."""

    report_paths = resolve_unpivot_report_paths(
        config,
        config_path,
        reports_dir,
...
def _write_errors_csv(path: Path, error_rows: list[UnpivotErrorRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
...
def _write_skipped_csv(path: Path, skipped_rows: list[UnpivotSkippedRow], run_id: str) -> None:
    rows = [
        {
            "run_id": run_id,
            "row_number": row.row_number,
...
def _error_row_payload(row: UnpivotErrorRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "input_name": row.input_name,
        "row_number": row.row_number,
...
def _skipped_row_payload(row: UnpivotSkippedRow, run_id: str) -> dict[str, Any]:
    return {
        "run_id": run_id,
        "row_number": row.row_number,
        "reason": row.reason,
