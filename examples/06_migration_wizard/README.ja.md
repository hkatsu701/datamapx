...
- 出力列数と列名の明示入力、schema override、`lookup`、`derived`、`expression`、`when`、`validations`、`filters`、`checks`
...
- `user_id`、`full_name`、`department_code`、`amount`、`status_code`、`active_flag` の input schema override
- 出力列 6 本の定義と、それぞれの rule
- `dept_code`、`dept_name`、`bonus_rate` の reference schema override
- 部署名と料率を取る `lookup`
- `derived.tax_amount`
