...
When `run` is invoked with `--limit N`, only the first `N` normalized input rows are loaded from the migration input CSV. Reference CSVs are still loaded fully. The CLI summary, `summary.json`, and `report.html` include `notes.limited_run: true` and `notes.limit: N` so the limited execution is visible in reports.
...
- `filters.include` and `filters.exclude` use the existing condition syntax and are applied after schema normalization, before unpivot expansion.
- Rows removed by filters are recorded in `skipped.csv`; an `exclude` rule can provide a custom `reason`.
